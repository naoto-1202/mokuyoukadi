...
$sql = 'SELECT bbs_entries.*, user.name AS user_name, users.icon_filename AS user_icon_filename'
  .  ' FROM bbs_entries'
  .  ' INNER JOIN users ON bbs_entries.user_id = users.id'
  .  ' WHERE '
  .  '   bbs_entries.user_id IN'
  .  '     (SELECT followee_user_id FROM user_realationships WHERE follower_user_id = :login_user_id) '
